...
function normalizeCompany(source: UnknownRecord, displayName: string): CompanyProfile['company'] {
  ...
  const addressLine =
    text(first(addressRecord, ['addressLine', 'line1', 'street'])) ??
    text(first(record, ['addressLine', 'streetAddress']))
  const city = text(first(addressRecord, ['city'])) ?? text(first(record, ['city']))
  const region =
    text(first(addressRecord, ['region', 'state'])) ??
    text(first(record, ['region', 'state']))
  const postalCode =
    text(first(addressRecord, ['postalCode', 'zipCode'])) ??
    text(first(record, ['postalCode', 'zipCode']))
  const country =
    text(first(addressRecord, ['country', 'countryCode'])) ??
    text(first(record, ['country']))
  const workingHoursStart =
    text(first(hoursRecord, ['start', 'startTime'])) ??
    text(first(record, ['workingHoursStart', 'companyStartTime']))
  const workingHoursEnd =
    text(first(hoursRecord, ['end', 'endTime'])) ??
    text(first(record, ['workingHoursEnd', 'companyEndTime']))
  const timezone =
    text(first(hoursRecord, ['timezone'])) ?? text(first(record, ['timezone']))
  ...
  return {
    ...
    ...(vatRegisteredValue !== undefined
      ? { vatRegistered: booleanValue(vatRegisteredValue, false) }
      : {}),
