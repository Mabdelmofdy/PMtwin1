...
describe('hiring application workflow', () => {
  ...
  it('runs application hiring path through contract creation with audit trail', () => {
    ...
    assert.ok(auditActions.includes('commercial_agreement.created_from_application'))
    assert.ok(auditActions.includes('contract.created_from_commercial_agreement'))
  })
})
